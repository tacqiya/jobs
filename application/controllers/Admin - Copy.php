...
class Admin extends CI_Controller
{
			...
			foreach ($sheetData as $worksheet) {
				if ($row >= 4) {
					...
					$insertData = [
						...
						'org_name' => $worksheet[5],
						'sector_name' => $worksheet[6],
						'division_name' => $worksheet[7],
						'dept_name' => $worksheet[8],
						'college' => $worksheet[8],
						'recruiter_name' => $worksheet[9],
						'recruiter_email' => $worksheet[10],
						'hiring_manager_name' => $worksheet[11],
						'hiring_manager_email' => $worksheet[12],
						'apply_link' => $worksheet[13],
						'project_title' => $worksheet[14],
						'project_manager_name' => $worksheet[15],
						'project_manager_email' => $worksheet[16],
						'emp_end_date' => $worksheet[17],
						'project_code' => $worksheet[18],
						'project_auth_name' => $worksheet[19],
						'project_auth_email' => $worksheet[20],
						'date_posted' => $worksheet[21],
						'closing_date' => $worksheet[22],
						'slug' => uniqid(uniqid()),
	...
	public function jd_import()
	{
		...
		if (isset($_FILES["file"]["name"])) {
			...
			$spreadsheet = $reader->load($_FILES['file']['tmp_name']);_e($spreadsheet);exit;
			$sheetData = $spreadsheet->getActiveSheet()->toArray();
			$row = 1;
			$highestRow = count($sheetData);
			foreach ($sheetData as $worksheet) {
